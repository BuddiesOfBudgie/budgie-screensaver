...
enum {
	PROP_0, PROP_ACTIVE, PROP_SESSION_IDLE, PROP_ACTIVATION_ENABLED,
};
...
static void raise_property_type_error(DBusConnection* connection, DBusMessage* in_reply_to, const char* device_id) {
	...
	snprintf(
		buf, 511, "Type mismatch setting property with id %s", device_id
	);
	gs_debug (buf);

	reply = dbus_message_new_error(
		in_reply_to, TYPE_MISMATCH_ERROR, buf
	);
	if (reply == NULL) {
		g_error ("No memory");
	}
...
static DBusHandlerResult listener_get_active_time(
	GSListener* listener,
	DBusConnection* connection,
	DBusMessage* message
) {
...
static DBusHandlerResult do_introspect(DBusConnection* connection, DBusMessage* message, dbus_bool_t local_interface) {
	...
	xml = g_string_append(
		xml, "  <interface name=\""GS_INTERFACE"\">\n"
			 "    <method name=\"Lock\">\n"
			 "    </method>\n"
	...
	dbus_message_append_args(
		reply, DBUS_TYPE_STRING, &xml_string, DBUS_TYPE_INVALID);
...
static gboolean _listener_message_path_is_our_session(GSListener* listener, DBusMessage* message) {
	...
	if (listener->priv->have_systemd) {
		return g_str_has_prefix(ssid, SYSTEMD_LOGIND_SESSION_PATH "/") && strcmp(
			ssid + sizeof(SYSTEMD_LOGIND_SESSION_PATH), listener->priv->session_id
		) == 0;
	}
#endif
...
static DBusHandlerResult listener_dbus_handle_system_message(
	...
	if (listener->priv->have_systemd) {
		...
		if (dbus_message_is_signal(message, SYSTEMD_LOGIND_SESSION_INTERFACE, "Unlock")) {
			...
			gboolean active;
			if (dbus_message_get_args(
				message, NULL, DBUS_TYPE_BOOLEAN, &active, DBUS_TYPE_INVALID) && active) {
	...
	if (dbus_message_is_signal(message, CK_SESSION_INTERFACE, "Unlock")) {
		...
		if (_listener_message_path_is_our_session(listener, message)) {
			...
			dbus_error_init(&error);
			if (dbus_message_get_args(
				message, &error, DBUS_TYPE_BOOLEAN, &new_active, DBUS_TYPE_INVALID)) {
...
static DBusHandlerResult gs_listener_message_handler(
	DBusConnection* connection,
	DBusMessage* message,
	void* user_data
) {
...
static DBusHandlerResult listener_dbus_filter_function(
	...
	*/
	...
	if (dbus_message_is_signal(message, DBUS_INTERFACE_LOCAL, "Disconnected") && strcmp(path, DBUS_PATH_LOCAL) == 0) {
		...
		message, DBUS_INTERFACE_DBUS, "NameLost"
	)) {
		const char* name;
		DBusError error;
		dbus_error_init(&error);
		...
		if (!dbus_message_get_args(
			message, &error, DBUS_TYPE_STRING, &name, DBUS_TYPE_INVALID)) {
...
static DBusHandlerResult listener_dbus_system_filter_function(
	DBusConnection* connection,
	DBusMessage* message,
	void* user_data
) {
...
static void gs_listener_class_init(GSListenerClass* klass) {
	...
	signals[CONFIG_LOCK] = g_signal_new(
		"config-lock",
		G_TYPE_FROM_CLASS (object_class),
		G_SIGNAL_RUN_LAST,
	...
	signals[SIMULATE_USER_ACTIVITY] = g_signal_new(
	...
	signals[ACTIVE_CHANGED] = g_signal_new(
		...
		1,
		G_TYPE_BOOLEAN);
	signals[SHOW_MESSAGE] = g_signal_new(
		"show-message",
		G_TYPE_FROM_CLASS (object_class),
		G_SIGNAL_RUN_LAST,
	...
	g_object_class_install_property(
		object_class, PROP_ACTIVE, g_param_spec_boolean(
			"active", NULL, NULL, FALSE, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_ACTIVATION_ENABLED, g_param_spec_boolean(
			"activation-enabled", NULL, NULL, TRUE, G_PARAM_READWRITE
		));
}
...
gboolean gs_listener_acquire(GSListener* listener, GError** error) {
	...
	if (dbus_connection_register_object_path(
		listener->priv->connection, GS_PATH, &gs_listener_vtable, listener
	) == FALSE) {
	...
	if (dbus_error_is_set(&buserror)) {
		g_set_error(
			error, GS_LISTENER_ERROR, GS_LISTENER_ERROR_ACQUISITION_FAILURE, "%s", buserror.message
		);
	}
	...
	dbus_bus_add_match(
		listener->priv->connection, "type='signal'"
									",interface='"DBUS_INTERFACE_DBUS"'"
									",sender='"DBUS_SERVICE_DBUS"'"
									",member='NameLost'", NULL);

	if (listener->priv->system_connection != NULL) {
		dbus_connection_add_filter(
			listener->priv->system_connection, listener_dbus_system_filter_function, listener, NULL);
#ifdef WITH_SYSTEMD
		if (listener->priv->have_systemd) {
			dbus_bus_add_match(
				listener->priv->system_connection, "type='signal',"
		...
		dbus_bus_add_match(
			...
			"type='signal',interface='"CK_SESSION_INTERFACE"',member='Unlock'",
			NULL);
		dbus_bus_add_match(
			listener->priv->system_connection, "type='signal',interface='"CK_SESSION_INTERFACE"',member='Lock'", NULL);
		dbus_bus_add_match(
			listener->priv->system_connection,
			"type='signal',interface='"CK_SESSION_INTERFACE"',member='ActiveChanged'",
			NULL);
#endif
	}
