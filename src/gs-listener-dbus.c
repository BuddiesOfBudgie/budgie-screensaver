...
static void gs_listener_unregister_handler(DBusConnection* connection, void* data);

static DBusHandlerResult gs_listener_message_handler(DBusConnection* connection, DBusMessage* message, void* user_data);
...
enum { LOCK, CONFIG_LOCK, QUIT, SIMULATE_USER_ACTIVITY, ACTIVE_CHANGED, SHOW_MESSAGE, LAST_SIGNAL };
...
static DBusObjectPathVTable gs_listener_vtable = {
	&gs_listener_unregister_handler, &gs_listener_message_handler, NULL, NULL, NULL, NULL};
...
static gboolean send_dbus_message(DBusConnection* connection, DBusMessage* message) {
...
static void send_dbus_boolean_signal(GSListener* listener, const char* name, gboolean value) {
	DBusMessage* message;
	DBusMessageIter iter;

	g_return_if_fail(listener != NULL);

	message = dbus_message_new_signal(GS_PATH, GS_SERVICE, name);
...
static void gs_listener_send_signal_active_changed(GSListener* listener) {
	g_return_if_fail(listener != NULL);

	gs_debug("Sending the ActiveChanged(%s) signal on the session bus", listener->priv->active ? "TRUE" : "FALSE");

	send_dbus_boolean_signal(listener, "ActiveChanged", listener->priv->active);
}

static gboolean listener_check_activation(GSListener* listener) {
	gboolean res;
...
static gboolean listener_set_session_idle_internal(GSListener* listener, gboolean idle) {
...
static gboolean listener_set_active_internal(GSListener* listener, gboolean active) {
...
gboolean gs_listener_set_active(GSListener* listener, gboolean active) {
	gboolean res;

	g_return_val_if_fail(GS_IS_LISTENER(listener), FALSE);

	if (listener->priv->active == active) {
		gs_debug("Trying to set active state when already: %s", active ? "active" : "inactive");
		return FALSE;
	}
...
gboolean gs_listener_set_session_idle(GSListener* listener, gboolean idle) {
	gboolean res;
	...
	if (listener->priv->session_idle == idle) {
		gs_debug("Trying to set idle state when already %s", idle ? "idle" : "not idle");
		return FALSE;
	}
...
void gs_listener_set_activation_enabled(GSListener* listener, gboolean enabled) {
...
static dbus_bool_t listener_property_set_bool(GSListener* listener, guint prop_id, dbus_bool_t value) {
	dbus_bool_t ret;
...
static void raise_error(DBusConnection* connection, DBusMessage* in_reply_to, const char* error_name, char* format, ...) {
	char buf[512];
	DBusMessage* reply;
...
static void raise_syntax(DBusConnection* connection, DBusMessage* in_reply_to, const char* method_name) {
	raise_error(connection, in_reply_to, GS_SERVICE ".SyntaxError", "There is a syntax error in the invocation of the method %s",
		method_name);
}

static void raise_property_type_error(DBusConnection* connection, DBusMessage* in_reply_to, const char* device_id) {
	char buf[512];
	DBusMessage* reply;
	...
	reply = dbus_message_new_error(in_reply_to, TYPE_MISMATCH_ERROR, buf);
	if (reply == NULL) {
		g_error("No memory");
	}
	if (!dbus_connection_send(connection, reply, NULL)) {
...
static DBusHandlerResult listener_set_property(
	GSListener* listener, DBusConnection* connection, DBusMessage* message, guint prop_id) {
	const char* path;
	int type;
	gboolean rc;
	DBusMessageIter iter;
...
static DBusHandlerResult listener_get_property(
	GSListener* listener, DBusConnection* connection, DBusMessage* message, guint prop_id) {
	DBusMessageIter iter;
	DBusMessage* reply;
	...
	if (reply == NULL) g_error("No memory");
...
static DBusHandlerResult listener_get_active_time(GSListener* listener, DBusConnection* connection, DBusMessage* message) {
	DBusMessageIter iter;
	DBusMessage* reply;
	dbus_uint32_t secs;
...
static DBusHandlerResult listener_show_message(GSListener* listener, DBusConnection* connection, DBusMessage* message) {
	DBusMessageIter iter;
	DBusMessage* reply;
	DBusError error;
	...
	if (listener->priv->active) {
		...
		if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &summary, DBUS_TYPE_STRING, &body, DBUS_TYPE_STRING, &icon,
				DBUS_TYPE_INVALID)) {
...
static DBusHandlerResult do_introspect(DBusConnection* connection, DBusMessage* message, dbus_bool_t local_interface) {
	DBusMessage* reply;
	GString* xml;
	char* xml_string;
	...
	xml = g_string_append(xml, "  <interface name=\"" GS_INTERFACE
							   "\">\n"
							   "    <method name=\"Lock\">\n"
							   "    </method>\n"
	...
	dbus_message_append_args(reply, DBUS_TYPE_STRING, &xml_string, DBUS_TYPE_INVALID);
...
static DBusHandlerResult send_success_reply(DBusConnection* connection, DBusMessage* message) {
	DBusMessage* reply;
...
static DBusHandlerResult listener_dbus_handle_session_message(
	DBusConnection* connection, DBusMessage* message, void* user_data, dbus_bool_t local_interface) {
...
static gboolean _listener_message_path_is_our_session(GSListener* listener, DBusMessage* message) {
	const char* ssid;

	ssid = dbus_message_get_path(message);
	if (ssid == NULL) return FALSE;

	if (listener->priv->session_id == NULL) return FALSE;
	...
	if (listener->priv->have_systemd)
		return g_str_has_prefix(ssid, SYSTEMD_LOGIND_SESSION_PATH "/") &&
			   strcmp(ssid + sizeof(SYSTEMD_LOGIND_SESSION_PATH), listener->priv->session_id) == 0;
#endif

#ifdef WITH_CONSOLE_KIT
	if (strcmp(ssid, listener->priv->session_id) == 0) return TRUE;
#endif
...
#ifdef WITH_SYSTEMD
static gboolean properties_changed_match(DBusMessage* message, const char* property) {
	DBusMessageIter iter, sub, sub2;

	/* Checks whether a certain property is listed in the
	 * specified PropertiesChanged message */

	if (!dbus_message_iter_init(message, &iter)) goto failure;

	/* Jump over interface name */
	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_STRING) goto failure;
	...
	while (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_INVALID) {
		...
		if (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_DICT_ENTRY) goto failure;

		dbus_message_iter_recurse(&sub, &sub2);
		dbus_message_iter_get_basic(&sub2, &name);

		if (strcmp(name, property) == 0) return TRUE;
	...
	while (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_INVALID) {
		...
		if (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_STRING) goto failure;
...
static DBusHandlerResult listener_dbus_handle_system_message(
	DBusConnection* connection, DBusMessage* message, void* user_data, dbus_bool_t local_interface) {
	GSListener* listener = GS_LISTENER(user_data);
...
#if 1
	gs_debug("obj_path=%s interface=%s method=%s destination=%s", dbus_message_get_path(message),
		dbus_message_get_interface(message), dbus_message_get_member(message), dbus_message_get_destination(message));
#endif
	...
	if (listener->priv->have_systemd) {
		if (dbus_message_is_signal(message, SYSTEMD_LOGIND_SESSION_INTERFACE, "Unlock")) {
			...
			gboolean active;
			if (dbus_message_get_args(message, NULL, DBUS_TYPE_BOOLEAN, &active, DBUS_TYPE_INVALID) && active) {
			...
			if (_listener_message_path_is_our_session(listener, message)) {
				if (properties_changed_match(message, "Active")) {
					...
					if (new_active) g_signal_emit(listener, signals[SIMULATE_USER_ACTIVITY], 0);
				}
			}
	...
	if (dbus_message_is_signal(message, CK_SESSION_INTERFACE, "Unlock")) {
		...
		if (_listener_message_path_is_our_session(listener, message)) {
			...
			dbus_error_init(&error);
			if (dbus_message_get_args(message, &error, DBUS_TYPE_BOOLEAN, &new_active, DBUS_TYPE_INVALID)) {
...
static gboolean gs_listener_dbus_init(GSListener* listener) {
	DBusError error;
	...
	if (listener->priv->connection == NULL) {
		...
		if (listener->priv->connection == NULL) {
			if (dbus_error_is_set(&error)) {
				gs_debug("couldn't connect to session bus: %s", error.message);
				dbus_error_free(&error);
			}
			return FALSE;
		}
	...
	if (listener->priv->system_connection == NULL) {
		...
		if (listener->priv->system_connection == NULL) {
			if (dbus_error_is_set(&error)) {
				gs_debug("couldn't connect to system bus: %s", error.message);
				dbus_error_free(&error);
			}
			return FALSE;
		}
...
static gboolean reinit_dbus(GSListener* listener) {
...
static DBusHandlerResult listener_dbus_filter_function(DBusConnection* connection, DBusMessage* message, void* user_data) {
	GSListener* listener = GS_LISTENER(user_data);
	const char* path;
	...
	*/
	...
	if (dbus_message_is_signal(message, DBUS_INTERFACE_LOCAL, "Disconnected") && strcmp(path, DBUS_PATH_LOCAL) == 0) {
		g_message(
			"Got disconnected from the session message bus; "
...
static DBusHandlerResult listener_dbus_system_filter_function(DBusConnection* connection, DBusMessage* message, void* user_data) {
	GSListener* listener = GS_LISTENER(user_data);
	const char* path;
...
static void gs_listener_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
	GSListener* self;
...
static void gs_listener_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
	GSListener* self;
...
static void gs_listener_class_init(GSListenerClass* klass) {
	...
	signals[LOCK] = g_signal_new("lock", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, lock), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	signals[CONFIG_LOCK] = g_signal_new("config-lock", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, config_lock), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	signals[QUIT] = g_signal_new("quit", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, quit), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	signals[SIMULATE_USER_ACTIVITY] = g_signal_new("simulate-user-activity", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, simulate_user_activity), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	signals[ACTIVE_CHANGED] = g_signal_new("active-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, active_changed), NULL, NULL, gs_marshal_BOOLEAN__BOOLEAN, G_TYPE_BOOLEAN, 1,
		G_TYPE_BOOLEAN);
	signals[SHOW_MESSAGE] = g_signal_new("show-message", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSListenerClass, show_message), NULL, NULL, gs_marshal_VOID__STRING_STRING_STRING, G_TYPE_NONE, 3,
		G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	g_object_class_install_property(
		object_class, PROP_ACTIVE, g_param_spec_boolean("active", NULL, NULL, FALSE, G_PARAM_READWRITE));
	g_object_class_install_property(
		object_class, PROP_ACTIVATION_ENABLED, g_param_spec_boolean("activation-enabled", NULL, NULL, TRUE, G_PARAM_READWRITE));

	g_type_class_add_private(klass, sizeof(GSListenerPrivate));
}


static gboolean screensaver_is_running(DBusConnection* connection) {
	DBusError error;
	gboolean exists;
...
gboolean gs_listener_acquire(GSListener* listener, GError** error) {
	int res;
	DBusError buserror;
	gboolean is_connected;
	...
	if (!listener->priv->connection) {
		g_set_error(
			error, GS_LISTENER_ERROR, GS_LISTENER_ERROR_ACQUISITION_FAILURE, "%s", _("failed to register with the message bus"));
		return FALSE;
	}

	is_connected = dbus_connection_get_is_connected(listener->priv->connection);
	if (!is_connected) {
		g_set_error(error, GS_LISTENER_ERROR, GS_LISTENER_ERROR_ACQUISITION_FAILURE, "%s", _("not connected to the message bus"));
		return FALSE;
	}

	dbus_error_init(&buserror);

	if (dbus_connection_register_object_path(listener->priv->connection, GS_PATH, &gs_listener_vtable, listener) == FALSE) {
	...
	if (dbus_error_is_set(&buserror)) {
		g_set_error(error, GS_LISTENER_ERROR, GS_LISTENER_ERROR_ACQUISITION_FAILURE, "%s", buserror.message);
	}
	if (res == DBUS_REQUEST_NAME_REPLY_EXISTS) {
		g_set_error(error, GS_LISTENER_ERROR, GS_LISTENER_ERROR_ACQUISITION_FAILURE, "%s",
...
static char* query_session_id(GSListener* listener) {
	DBusMessage* message;
	DBusMessage* reply;
	DBusError error;
...
static void init_session_id(GSListener* listener) {
