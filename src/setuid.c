...
static char* uid_gid_string(uid_t uid, gid_t gid) {
	...
	buf = g_strdup_printf("%s/%s (%ld/%ld)", (p && p->pw_name ? p->pw_name : "???"), (g && g->gr_name ? g->gr_name : "???"),
		(long) uid, (long) gid);
...
static gboolean set_ids_by_number(uid_t uid, gid_t gid, char** message_ret) {
	...
	if (message_ret) *message_ret = NULL;
	...
	*/
	...
	errno = 0;
	if (setgroups(1, &gid) < 0) sgs_errno = errno ? errno : -1;

	errno = 0;
	if (setgid(gid) != 0) gid_errno = errno ? errno : -1;

	errno = 0;
	if (setuid(uid) != 0) uid_errno = errno ? errno : -1;

	if (uid_errno == 0 && gid_errno == 0 && sgs_errno == 0) {
		static char* reason;
		reason = g_strdup_printf("changed uid/gid to %s/%s (%ld/%ld).", (p && p->pw_name ? p->pw_name : "???"),
			(g && g->gr_name ? g->gr_name : "???"), (long) uid, (long) gid);
		if (message_ret) *message_ret = g_strdup(reason);
		...
		if (sgs_errno) {
			reason = g_strdup_printf("couldn't setgroups to %s (%ld)", (g && g->gr_name ? g->gr_name : "???"), (long) gid);
			if (sgs_errno == -1)
		...
		if (gid_errno) {
			reason = g_strdup_printf("couldn't set gid to %s (%ld)", (g && g->gr_name ? g->gr_name : "???"), (long) gid);
			if (gid_errno == -1)
				fprintf(stderr, "%s: unknown error\n", reason);
			else {
				errno = gid_errno;
		...
		if (uid_errno) {
			reason = g_strdup_printf("couldn't set uid to %s (%ld)", (p && p->pw_name ? p->pw_name : "???"), (long) uid);
			if (uid_errno == -1)
				fprintf(stderr, "%s: unknown error\n", reason);
			else {
				errno = uid_errno;
