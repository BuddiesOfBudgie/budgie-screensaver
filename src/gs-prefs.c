...
static void gs_prefs_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
	switch (prop_id) {
		default:
...
static void gs_prefs_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
	switch (prop_id) {
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
			break;
...
static void gs_prefs_class_init(GSPrefsClass* klass) {
	...
	signals[CHANGED] = g_signal_new("changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSPrefsClass, changed), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

	g_type_class_add_private(klass, sizeof(GSPrefsPrivate));
}

static void _gs_prefs_set_lock_timeout(GSPrefs* prefs, guint value) {
...
static void _gs_prefs_set_idle_activation_enabled(GSPrefs* prefs, gboolean value) {
	prefs->idle_activation_enabled = value;
}

static void _gs_prefs_set_lock_enabled(GSPrefs* prefs, gboolean value) {
	prefs->lock_enabled = value;
}

static void _gs_prefs_set_lock_disabled(GSPrefs* prefs, gboolean value) {
	prefs->lock_disabled = value;
}

static void _gs_prefs_set_user_switch_disabled(GSPrefs* prefs, gboolean value) {
	prefs->user_switch_disabled = value;
}

static void _gs_prefs_set_keyboard_enabled(GSPrefs* prefs, gboolean value) {
	prefs->keyboard_enabled = value;
}

static void _gs_prefs_set_keyboard_command(GSPrefs* prefs, const char* value) {
...
static void _gs_prefs_set_status_message_enabled(GSPrefs* prefs, gboolean enabled) {
	prefs->status_message_enabled = enabled;
}

static void _gs_prefs_set_logout_enabled(GSPrefs* prefs, gboolean value) {
	prefs->logout_enabled = value;
}

static void _gs_prefs_set_logout_command(GSPrefs* prefs, const char* value) {
...
static void _gs_prefs_set_logout_timeout(GSPrefs* prefs, guint value) {
	/* prevent overflow when converting to milliseconds */
	if (value > G_MAXUINT / 1000) {
		value = G_MAXUINT / 1000;
	}
...
static void _gs_prefs_set_user_switch_enabled(GSPrefs* prefs, gboolean value) {
	prefs->user_switch_enabled = value;
}

static guint _gs_settings_get_uint(GSettings* settings, const char* key) {
	guint value;
...
static void gs_prefs_load_from_settings(GSPrefs* prefs) {
	guint uvalue;
	gboolean bvalue;
	char* string;
...
static void key_changed_cb(GSettings* settings, const gchar* key, GSPrefs* prefs) {
...
static void gs_prefs_init(GSPrefs* prefs) {
	...
	g_signal_connect(prefs->priv->settings, "changed", G_CALLBACK(key_changed_cb), prefs);
	prefs->priv->lockdown = g_settings_new(LOCKDOWN_SETTINGS_SCHEMA);
	g_signal_connect(prefs->priv->lockdown, "changed", G_CALLBACK(key_changed_cb), prefs);
