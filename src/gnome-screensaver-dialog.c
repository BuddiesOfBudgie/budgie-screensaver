...
static GOptionEntry entries[] = {{"verbose", 0, 0, G_OPTION_ARG_NONE, &verbose, N_("Show debugging output"), NULL},
	...
	{"status-message", 0, 0, G_OPTION_ARG_STRING, &status_message, N_("Message to show in the dialog"),
		/* Translators: This is the example input for the --status-message command line option. */
		N_("MESSAGE")},
	{"away-message", 0, 0, G_OPTION_ARG_STRING, &away_message, N_("Not used"),
...
static const char* maybe_translate_message(const char* msg) {
	...
	if (hash == NULL) {
		...
		g_hash_table_insert(
			hash, "Password has been already used. Choose another.", _("Password has been already used. Choose another."));
		g_hash_table_insert(
			hash, "You must wait longer to change your password", _("You must wait longer to change your password"));
