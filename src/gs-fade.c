...
static gboolean xf86_whack_gamma(int screen, struct GSFadeScreenPrivate* screen_priv, float ratio) {
	...
	if (gamma_info->size == 0) {
		...
		gdk_x11_display_error_trap_push(default_display);
		status = XF86VidModeSetGammaRamp(
...
static gboolean gamma_fade_setup(GSFade* fade, int screen_idx) {
	...
	if (FADE_TYPE_GAMMA_RAMP == screen_priv->fade_type) {
		...
		res = XF86VidModeGetGammaRampSize(
			GDK_DISPLAY_XDISPLAY (gdk_display_get_default()),
			screen_idx,
...
test_number:
	if (FADE_TYPE_GAMMA_NUMBER == screen_priv->fade_type) {
		...
		gs_debug (
			"Initialized gamma fade for screen %d: %f %f %f",
			screen_idx,
			screen_priv->vmg.red,
...
static void check_gamma_extension(GSFade* fade, int screen_idx) {
	...
	if (major < XF86_VIDMODE_GAMMA_MIN_MAJOR
		|| (major == XF86_VIDMODE_GAMMA_MIN_MAJOR && minor < XF86_VIDMODE_GAMMA_MIN_MINOR)) {
		goto fade_none;
	}
