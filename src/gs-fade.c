...
struct GSFadeScreenPrivate {
	...
	gboolean (*fade_set_alpha_gamma)(GSFade* fade, int screen, gdouble alpha);
...
static Bool safe_XF86VidModeQueryVersion(Display* dpy, int* majP, int* minP) {
	Bool result;
	...
	return (error_handler_hit ? False : result);
}

static gboolean xf86_whack_gamma(int screen, struct GSFadeScreenPrivate* screen_priv, float ratio) {
	Bool status;
	struct GSGammaInfo* gamma_info;

	gamma_info = screen_priv->info;

	if (!gamma_info) return FALSE;
...
void gs_fade_set_enabled(GSFade* fade, gboolean enabled) {
...
#ifdef HAVE_XF86VMODE_GAMMA
static gboolean gamma_fade_setup(GSFade* fade, int screen_idx) {
	gboolean res;
	struct GSFadeScreenPrivate* screen_priv;

	screen_priv = &fade->priv->screen_priv[screen_idx];

	if (screen_priv->info) return TRUE;
	...
	if (FADE_TYPE_GAMMA_RAMP == screen_priv->fade_type) {
		...
		res = XF86VidModeGetGammaRamp(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), screen_idx, screen_priv->info->size,
			screen_priv->info->r, screen_priv->info->g, screen_priv->info->b);
		if (!res) {
			screen_priv->fade_type = FADE_TYPE_GAMMA_NUMBER;
			goto test_number;
		}
...
test_number:
	if (FADE_TYPE_GAMMA_NUMBER == screen_priv->fade_type) {
		...
		gs_debug("Initialized gamma fade for screen %d: %f %f %f", screen_idx, screen_priv->vmg.red, screen_priv->vmg.green,
...
static void screen_fade_finish(GSFade* fade, int screen_idx) {
	struct GSFadeScreenPrivate* screen_priv;
	int i;
	screen_priv = &fade->priv->screen_priv[screen_idx];

	if (!screen_priv->info) return;

	for (i = 0; i < screen_priv->num_ramps; i++) {
		if (screen_priv->info[i].r) g_free(screen_priv->info[i].r);
		if (screen_priv->info[i].g) g_free(screen_priv->info[i].g);
		if (screen_priv->info[i].b) g_free(screen_priv->info[i].b);
	}
...
#ifdef HAVE_XF86VMODE_GAMMA
static gboolean gamma_fade_set_alpha_gamma(GSFade* fade, int screen_idx, gdouble alpha) {
	struct GSFadeScreenPrivate* screen_priv;
...
static void check_gamma_extension(GSFade* fade, int screen_idx) {
	struct GSFadeScreenPrivate* screen_priv;
#ifdef HAVE_XF86VMODE_GAMMA
	int event;
	int error;
	...
	if (!res) goto fade_none;

	res = safe_XF86VidModeQueryVersion(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), &major, &minor);
	if (!res) goto fade_none;

	if (major < XF86_VIDMODE_GAMMA_MIN_MAJOR || (major == XF86_VIDMODE_GAMMA_MIN_MAJOR && minor < XF86_VIDMODE_GAMMA_MIN_MINOR))
		goto fade_none;
	...
	if (major < XF86_VIDMODE_GAMMA_RAMP_MIN_MAJOR ||
		(major == XF86_VIDMODE_GAMMA_RAMP_MIN_MAJOR && minor < XF86_VIDMODE_GAMMA_RAMP_MIN_MINOR)) {
		screen_priv->fade_type = FADE_TYPE_GAMMA_NUMBER;
		return;
	}
...
static gboolean xrandr_fade_setup(GSFade* fade, int screen_idx) {
	...
	while (*crtcs) {
		...
		if (!gnome_rr_crtc_get_current_mode(crtc)) {
			...
			res = gnome_rr_crtc_get_gamma(crtc, &info->size, &info->r, &info->g, &info->b);
			if (res == FALSE) goto fail;
		}
...
static void xrandr_crtc_whack_gamma(GnomeRRCrtc* crtc, struct GSGammaInfo* gamma_info, float ratio) {
	unsigned short *r, *g, *b;
	int i;

	if (gamma_info->size == 0) return;
	...
	gnome_rr_crtc_set_gamma(crtc, gamma_info->size, r, g, b);
	g_free(r);
	g_free(g);
	g_free(b);
}

static gboolean xrandr_fade_set_alpha_gamma(GSFade* fade, int screen_idx, gdouble alpha) {
	struct GSFadeScreenPrivate* screen_priv;
	struct GSGammaInfo* info;
	GnomeRRCrtc** crtcs;
	int i;
	...
	if (!screen_priv->info) return FALSE;
...
static void check_randr_extension(GSFade* fade, int screen_idx) {
	...
	screen_priv->rrscreen = gnome_rr_screen_new(screen, NULL);
...
static gboolean gs_fade_set_alpha(GSFade* fade, gdouble alpha) {
	gboolean ret = FALSE;
	int i;
...
static gboolean gs_fade_out_iter(GSFade* fade) {
	gboolean ret;
...
static gboolean gs_fade_stop(GSFade* fade) {
...
static gboolean fade_out_timer(GSFade* fade) {
	gboolean res;
...
static void gs_fade_set_timeout(GSFade* fade, guint timeout) {
	g_return_if_fail(GS_IS_FADE(fade));

	fade->priv->timeout = timeout;
}

static void gs_fade_start(GSFade* fade, guint timeout) {
	...
	for (i = 0; i < fade->priv->num_screens; i++) {
		screen_priv = &fade->priv->screen_priv[i];
		if (screen_priv->fade_type != FADE_TYPE_NONE) active_fade = TRUE;
	}
	if (active_fade) {
		guint num_steps;
...
static void gs_fade_async_callback(GSFade* fade, FadedCallbackData* cdata) {
	g_signal_handlers_disconnect_by_func(fade, gs_fade_async_callback, cdata);
...
void gs_fade_async(GSFade* fade, guint timeout, GSFadeDoneFunc func, gpointer data) {
	g_return_if_fail(GS_IS_FADE(fade));
	...
	if (func) {
		...
		g_signal_connect(fade, "faded", G_CALLBACK(gs_fade_async_callback), cb_data);
	}

	gs_fade_start(fade, timeout);
}

static void gs_fade_sync_callback(GSFade* fade, int* flag) {
	*flag = TRUE;
	g_signal_handlers_disconnect_by_func(fade, gs_fade_sync_callback, flag);
}

void gs_fade_sync(GSFade* fade, guint timeout) {
	int flag = FALSE;
	...
	g_signal_connect(fade, "faded", G_CALLBACK(gs_fade_sync_callback), &flag);
...
static void gs_fade_class_init(GSFadeClass* klass) {
	...
	signals[FADED] = g_signal_new("faded", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSFadeClass, faded), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
...
static void gs_fade_init(GSFade* fade) {
	...
	for (i = 0; i < fade->priv->num_screens; i++) {
		...
		if (!fade->priv->screen_priv[i].fade_type) check_gamma_extension(fade, i);
...
static void gs_fade_finalize(GObject* object) {
	...
	for (i = 0; i < fade->priv->num_screens; i++) fade->priv->screen_priv[i].fade_finish(fade, i);

	if (fade->priv->screen_priv) {
		for (i = 0; i < fade->priv->num_screens; i++) {
			if (!fade->priv->screen_priv[i].rrscreen) continue;
...
GSFade* gs_fade_new(void) {
	if (fade_object) {
		...
		g_object_add_weak_pointer(fade_object, (gpointer*) &fade_object);
	}
