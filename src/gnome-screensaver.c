...
int main(int argc, char** argv) {
	...
	static GOptionEntry entries[] = {{"version", 0, 0, G_OPTION_ARG_NONE, &show_version, N_("Version of this application"), NULL},
	...
	if (g_find_program_in_path("pkill") != NULL) { // Have pkill
		...
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 5, attempt_gjs_screensaver_kill, NULL,
			NULL); // Basically have a defer after 5s to attempt another kill of GJS
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 10, attempt_gjs_screensaver_kill, NULL,
			NULL); // Basically have a defer after 10s to attempt another kill of GJS
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 30, attempt_gjs_screensaver_kill, NULL,
			NULL); // In the event gjs is still going after 30s, kill again
	}
