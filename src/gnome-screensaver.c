...
int main(int argc, char** argv) {
	...
	static GOptionEntry entries[] =
		{{"version", 0, 0, G_OPTION_ARG_NONE, &show_version, N_("Version of this application"), NULL},
			...
			{"debug", 0, 0, G_OPTION_ARG_NONE, &debug, N_("Enable debugging code"), NULL}, {NULL}};
	...
	// While the logic might seem weird, this saves us from having to implement a reading of /proc looking for gjs
	// running the org.gnome.ScreenSaver "service", killing it, then doing a defer and kill again in case it starts up
	// again. It also saves us from having to add any exponential backoff or logic for stopping the timer after N
	// attempts. Mostly just a stopgap to deal with situations where gnome-session is changed to either autostart or not
	// autostart dbus interfaces and weirdness where people use GDM with Budgie.
	if (g_find_program_in_path("pkill") != NULL) { // Have pkill
		attempt_gjs_screensaver_kill(NULL); // Attempt kill immediately

		// Basically have a defer after 5s to attempt another kill of GJS
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 5, attempt_gjs_screensaver_kill, NULL, NULL);

		// Basically have a defer after 10s to attempt another kill of GJS
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 10, attempt_gjs_screensaver_kill, NULL, NULL);

		// In the event gjs is still going after 30s, kill again
		g_timeout_add_seconds_full(G_PRIORITY_LOW, 30, attempt_gjs_screensaver_kill, NULL, NULL);
	}
