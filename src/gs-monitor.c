...
static void listener_config_lock_cb(GSListener* listener, GSMonitor* monitor) {
	(void) listener;
...
static void listener_show_message_cb(
	GSListener* listener,
	const char* summary,
	const char* body,
	const char* icon,
...
static void connect_listener_signals(GSMonitor* monitor) {
	...
	g_signal_connect (monitor->priv->listener, "config-lock", G_CALLBACK(listener_config_lock_cb), monitor);
	...
	g_signal_connect (monitor->priv->listener,
...
static void connect_watcher_signals(GSMonitor* monitor) {
	...
	g_signal_connect (
		monitor->priv->watcher,
		"notify::status-message",
		G_CALLBACK(on_watcher_status_message_changed),
		monitor
	);
}
