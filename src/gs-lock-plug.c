...
static void do_user_switch(GSLockPlug* plug) {
	...
	if (g_getenv("XDG_SEAT_PATH")) {
		...
		command = g_strdup_printf(
			"%s %s", GDM_FLEXISERVER_COMMAND, GDM_FLEXISERVER_ARGS
		);
...
static void gs_lock_plug_response(GSLockPlug* plug, gint response_id) {
	...
	g_signal_emit(
		plug, lock_plug_signals[RESPONSE], 0, new_response
	);
}
...
static gboolean dialog_timed_out(GSLockPlug* plug) {
	...
	plug->priv->response_idle_id = g_timeout_add_seconds(
		2, (GSourceFunc) response_cancel_idle_cb, plug
	);
	return FALSE;
}
...
static void restart_cancel_timeout(GSLockPlug* plug) {
	...
	plug->priv->cancel_timeout_id = g_timeout_add(
		plug->priv->timeout, (GSourceFunc) dialog_timed_out, plug
	);
}
...
static void rounded_rectangle(
	...
	cairo_arc(
		cr, x + width - radius, y + height - radius, radius, 0 * degrees, 90 * degrees
	);
	cairo_arc(
		cr, x + radius, y + height - radius, radius, 90 * degrees, 180 * degrees
	);
	cairo_arc(
		cr, x + radius, y + radius, radius, 180 * degrees, 270 * degrees
	);
	cairo_close_path(cr);
}
...
static void go_cairo_convert_data_to_pixbuf(
	unsigned char* dst,
...
static GdkPixbuf* frame_pixbuf(GdkPixbuf* source) {
	...
	surface = cairo_image_surface_create_for_data(
		data, CAIRO_FORMAT_ARGB32, w, h, rowstride
	);
	cr = cairo_create(surface);
	...
	rounded_rectangle(
		cr, 1.0, frame_width + 0.5, frame_width + 0.5, radius, w - frame_width * 2 - 1, h - frame_width * 2 - 1
	);
...
static GdkPixbuf* get_pixbuf_of_user_icon(GSLockPlug* plug) {
	...
	find_user_by_name_reply = g_dbus_connection_call_sync(
		system_bus,
	...
	user = g_variant_get_string(
		g_variant_get_child_value(
			find_user_by_name_reply, 0
		), NULL);

	get_icon_file_reply = g_dbus_connection_call_sync(
		system_bus, "org.freedesktop.Accounts", user, "org.freedesktop.DBus.Properties", "Get", g_variant_new(
			"(ss)", "org.freedesktop.Accounts.User", "IconFile"
...
static void queue_key_event(GSLockPlug* plug, GdkEventKey* event) {
	...
	plug->priv->key_events = g_list_prepend(
		plug->priv->key_events, saved_event
	);
}
...
static void forward_key_events(GSLockPlug* plug) {
	...
	while (plug->priv->key_events != NULL) {
		...
		plug->priv->key_events = g_list_delete_link(
			plug->priv->key_events, plug->priv->key_events
		);
	}
}
...
static void gs_lock_plug_class_init(GSLockPlugClass* klass) {
	...
	lock_plug_signals[RESPONSE] = g_signal_new(
		"response",
		...
		1,
		G_TYPE_INT);
	lock_plug_signals[CLOSE] = g_signal_new(
		"close",
		G_OBJECT_CLASS_TYPE (klass),
	...
	g_object_class_install_property(
		object_class, PROP_SWITCH_ENABLED, g_param_spec_boolean(
			"switch-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE
		));

	binding_set = gtk_binding_set_by_class(klass);

	gtk_binding_entry_add_signal(
		binding_set, GDK_KEY_Escape, 0, "close", 0
	);
}
...
static void logout_button_clicked(GtkButton* button, GSLockPlug* plug) {
	...
	g_spawn_async(
		g_get_home_dir(), argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, &error
	);
...
static void create_page_one_buttons(GSLockPlug* plug) {
	...
	plug->priv->auth_logout_button =
		gs_lock_plug_add_button(GS_LOCK_PLUG (plug), plug->priv->auth_action_area, _("Log _Out"));
	...
	plug->priv->auth_unlock_button =
		gs_lock_plug_add_button(GS_LOCK_PLUG (plug), plug->priv->auth_action_area, _("_Unlock"));
		...
		if (input_source->index == current) {
			if (!input_source->unique) {
				gchar* markup = g_markup_printf_escaped(
					"%s<sub><small>%u</small></sub>", input_source->name, input_source->subscript
				);
