...
static void gs_lock_plug_style_set(GtkWidget* widget, GtkStyle* previous_style) {
	GSLockPlug* plug;
...
static void do_user_switch(GSLockPlug* plug) {
	GError* error = NULL;
	...
	if (g_getenv("XDG_SEAT_PATH")) {
		...
		if (error) g_warning("Failed to get system bus: %s", error->message);
		g_clear_error(&error);

		if (bus)
			result = g_dbus_connection_call_sync(bus, "org.freedesktop.DisplayManager", g_getenv("XDG_SEAT_PATH"),
				"org.freedesktop.DisplayManager.Seat", "SwitchToGreeter", g_variant_new("()"), G_VARIANT_TYPE("()"),
				G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
		if (error) g_warning("Failed to switch to greeter: %s", error->message);
		g_clear_error(&error);
		...
		command = g_strdup_printf("%s %s", GDM_FLEXISERVER_COMMAND, GDM_FLEXISERVER_ARGS);
		...
		if (app) g_app_info_launch(app, NULL, context, &error);
...
static void set_status_text(GSLockPlug* plug, const char* text) {
	if (plug->priv->auth_message_label != NULL) {
		gtk_label_set_text(GTK_LABEL(plug->priv->auth_message_label), text);
	}
}

void gs_lock_plug_set_sensitive(GSLockPlug* plug, gboolean sensitive) {
...
static void remove_cancel_timeout(GSLockPlug* plug) {
...
static void remove_response_idle(GSLockPlug* plug) {
...
static void gs_lock_plug_response(GSLockPlug* plug, gint response_id) {
	int new_response;
	...
	g_signal_emit(plug, lock_plug_signals[RESPONSE], 0, new_response);
}

static gboolean response_cancel_idle_cb(GSLockPlug* plug) {
	plug->priv->response_idle_id = 0;
...
static gboolean dialog_timed_out(GSLockPlug* plug) {
	...
	plug->priv->response_idle_id = g_timeout_add_seconds(2, (GSourceFunc) response_cancel_idle_cb, plug);
	return FALSE;
}


static void kbd_lock_mode_update(GSLockPlug* plug, int mode) {
	...
	if ((mode & LOCK_CAPS) != 0) {
		gtk_label_set_text(GTK_LABEL(plug->priv->auth_capslock_label), _("You have the Caps Lock key on."));
	} else {
...
static void restart_cancel_timeout(GSLockPlug* plug) {
	remove_cancel_timeout(plug);

	plug->priv->cancel_timeout_id = g_timeout_add(plug->priv->timeout, (GSourceFunc) dialog_timed_out, plug);
}

void gs_lock_plug_get_text(GSLockPlug* plug, char** text) {
...
static void shutdown_loop(RunInfo* ri) {
	if (g_main_loop_is_running(ri->loop)) g_main_loop_quit(ri->loop);
}

static void run_unmap_handler(GSLockPlug* plug, gpointer data) {
	RunInfo* ri = data;

	shutdown_loop(ri);
}

static void run_response_handler(GSLockPlug* plug, gint response_id, gpointer data) {
	RunInfo* ri;
...
static gint run_delete_handler(GSLockPlug* plug, GdkEventAny* event, gpointer data) {
	RunInfo* ri = data;
...
static void run_destroy_handler(GSLockPlug* plug, gpointer data) {
	RunInfo* ri = data;
...
static void run_keymap_handler(GdkKeymap* keymap, GSLockPlug* plug) {
...
int gs_lock_plug_run(GSLockPlug* plug) {
	...
	keymap_handler = g_signal_connect(keymap, "state-changed", G_CALLBACK(run_keymap_handler), plug);

	response_handler = g_signal_connect(plug, "response", G_CALLBACK(run_response_handler), &ri);

	unmap_handler = g_signal_connect(plug, "unmap", G_CALLBACK(run_unmap_handler), &ri);

	delete_handler = g_signal_connect(plug, "delete_event", G_CALLBACK(run_delete_handler), &ri);

	destroy_handler = g_signal_connect(plug, "destroy", G_CALLBACK(run_destroy_handler), &ri);
...
static void rounded_rectangle(
	cairo_t* cr, gdouble aspect, gdouble x, gdouble y, gdouble corner_radius, gdouble width, gdouble height) {
	gdouble radius;
	gdouble degrees;
...
static void go_cairo_convert_data_to_pixbuf(unsigned char* dst, unsigned char const* src, int width, int height, int rowstride) {
	int i, j;
	unsigned int t;
	unsigned char a, b, c;
...
static void cairo_to_pixbuf(guint8* src_data, GdkPixbuf* dst_pixbuf) {
	unsigned char* src;
	unsigned char* dst;
	guint w;
	guint h;
...
static GdkPixbuf* frame_pixbuf(GdkPixbuf* source) {
	GdkPixbuf* dest;
	cairo_t* cr;
	cairo_surface_t* surface;
	guint w;
	...
	surface = cairo_image_surface_create_for_data(data, CAIRO_FORMAT_ARGB32, w, h, rowstride);
	cr = cairo_create(surface);
	...
	rounded_rectangle(cr, 1.0, frame_width + 0.5, frame_width + 0.5, radius, w - frame_width * 2 - 1, h - frame_width * 2 - 1);
...
static void image_set_from_pixbuf(GtkImage* image, GdkPixbuf* source) {
	GdkPixbuf* pixbuf;
...
static GdkPixbuf* get_pixbuf_of_user_icon(GSLockPlug* plug) {
	GError* error;
	...
	find_user_by_name_reply = g_dbus_connection_call_sync(system_bus, "org.freedesktop.Accounts", "/org/freedesktop/Accounts",
		"org.freedesktop.Accounts", "FindUserByName", g_variant_new("(s)", g_get_user_name()), G_VARIANT_TYPE("(o)"),
		G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
	if (error != NULL) {
	...
	user = g_variant_get_string(g_variant_get_child_value(find_user_by_name_reply, 0), NULL);

	get_icon_file_reply = g_dbus_connection_call_sync(system_bus, "org.freedesktop.Accounts", user,
		"org.freedesktop.DBus.Properties", "Get", g_variant_new("(ss)", "org.freedesktop.Accounts.User", "IconFile"),
	...
	if (icon_file == NULL) {
		...
		pixbuf = gdk_pixbuf_new_from_file_at_size(icon_file, 64, 64, &error);
	}
...
static gboolean set_face_image(GSLockPlug* plug) {
	GdkPixbuf* pixbuf;
...
static void gs_lock_plug_show(GtkWidget* widget) {
...
static void gs_lock_plug_hide(GtkWidget* widget) {
	if (GTK_WIDGET_CLASS(gs_lock_plug_parent_class)->hide) {
		GTK_WIDGET_CLASS(gs_lock_plug_parent_class)->hide(widget);
	}
}

static void queue_key_event(GSLockPlug* plug, GdkEventKey* event) {
...
static void forward_key_events(GSLockPlug* plug) {
...
static void gs_lock_plug_set_logout_enabled(GSLockPlug* plug, gboolean logout_enabled) {
...
static void gs_lock_plug_set_logout_command(GSLockPlug* plug, const char* command) {
	g_return_if_fail(GS_LOCK_PLUG(plug));
...
static void gs_lock_plug_set_status_message(GSLockPlug* plug, const char* status_message) {
	g_return_if_fail(GS_LOCK_PLUG(plug));
...
static void gs_lock_plug_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
	GSLockPlug* self;
...
static void gs_lock_plug_set_switch_enabled(GSLockPlug* plug, gboolean switch_enabled) {
	g_return_if_fail(GS_LOCK_PLUG(plug));
...
static void gs_lock_plug_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
	GSLockPlug* self;
...
static void gs_lock_plug_close(GSLockPlug* plug) {
...
static void gs_lock_plug_class_init(GSLockPlugClass* klass) {
	...
	lock_plug_signals[RESPONSE] = g_signal_new("response", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSLockPlugClass, response), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
	lock_plug_signals[CLOSE] = g_signal_new("close", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		G_STRUCT_OFFSET(GSLockPlugClass, close), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	...
	g_object_class_install_property(
		object_class, PROP_SWITCH_ENABLED, g_param_spec_boolean("switch-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE));
...
static void clear_clipboards(GSLockPlug* plug) {
...
static void logout_button_clicked(GtkButton* button, GSLockPlug* plug) {
	char** argv = NULL;
	GError* error = NULL;
	gboolean res;
	...
	g_spawn_async(g_get_home_dir(), argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, &error);
...
void gs_lock_plug_enable_prompt(GSLockPlug* plug, const char* message, gboolean visible) {
	char* markup;
...
void gs_lock_plug_show_message(GSLockPlug* plug, const char* message) {
	g_return_if_fail(GS_IS_LOCK_PLUG(plug));
...
static gint entry_key_press(GtkWidget* widget, GdkEventKey* event, GSLockPlug* plug) {
	restart_cancel_timeout(plug);

	/* if the input widget is visible and ready for input
	 * then just carry on as usual
	 */
	if (gtk_widget_get_visible(plug->priv->auth_prompt_entry) && gtk_widget_get_sensitive(plug->priv->auth_prompt_entry)) {
		return FALSE;
	}
...
static GtkWidget* gs_lock_plug_add_button(GSLockPlug* plug, GtkWidget* action_area, const gchar* button_text) {
	GtkWidget* button;
...
static void create_page_one_buttons(GSLockPlug* plug) {
	gs_profile_start("page one buttons");

	plug->priv->auth_switch_button =
		gs_lock_plug_add_button(GS_LOCK_PLUG(plug), plug->priv->auth_action_area, _("S_witch User…"));
	gtk_button_box_set_child_secondary(GTK_BUTTON_BOX(plug->priv->auth_action_area), plug->priv->auth_switch_button, TRUE);
	...
	plug->priv->auth_logout_button = gs_lock_plug_add_button(GS_LOCK_PLUG(plug), plug->priv->auth_action_area, _("Log _Out"));
	...
	plug->priv->auth_unlock_button = gs_lock_plug_add_button(GS_LOCK_PLUG(plug), plug->priv->auth_action_area, _("_Unlock"));
...
static void input_source_free(gpointer data) {
	InputSource* input_source = data;
	g_free(input_source->name);
	g_free(data);
}

static void input_sources_current_changed_cb(GSettings* settings, gchar* key, gpointer user_data) {
	...
	for (i = 0; i < plug->priv->input_sources->len; i++) {
		...
		if (input_source->index == current) {
			if (!input_source->unique) {
				gchar* markup =
					g_markup_printf_escaped("%s<sub><small>%u</small></sub>", input_source->name, input_source->subscript);
...
static void gs_lock_plug_init_input_sources(GSLockPlug* plug) {
	...
	if (plug->priv->input_sources != NULL) return;
	...
	for (i = 0; g_variant_iter_next(&iter, "(&s&s)", &type, &name); i++) {
		if (g_strcmp0(type, "xkb") == 0) {
			...
			if (input_source->index == current) plug->priv->input_source = plug->priv->input_sources->len;
	...
	g_signal_connect(
		plug->priv->input_sources_settings, "changed::" CURRENT_KEY, G_CALLBACK(input_sources_current_changed_cb), plug);
}

static gboolean layout_indicator_clicked_cb(GtkWidget* widget, GdkEvent* event, gpointer user_data) {
	GSLockPlug* plug;
	InputSource* input_source;
...
static void create_page_one(GSLockPlug* plug) {
	GtkWidget* align;
	GtkWidget* vbox;
	GtkWidget* vbox2;
	GtkWidget* hbox;
	...
	gtk_label_set_mnemonic_widget(GTK_LABEL(plug->priv->auth_prompt_label), plug->priv->auth_prompt_entry);
	...
	if (plug->priv->input_sources->len > 1) {
		...
		gtk_box_pack_start(GTK_BOX(hbox2), layout_indicator, FALSE, FALSE, 6);
	...
	gtk_box_pack_start(GTK_BOX(vbox), plug->priv->auth_message_label, FALSE, FALSE, 0);
	/* Buttons */
	plug->priv->auth_action_area = gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);

	gtk_button_box_set_layout(GTK_BUTTON_BOX(plug->priv->auth_action_area), GTK_BUTTONBOX_END);

	gtk_box_pack_end(GTK_BOX(vbox), plug->priv->auth_action_area, FALSE, TRUE, 0);
...
static void unlock_button_clicked(GtkButton* button, GSLockPlug* plug) {
	gs_lock_plug_response(plug, GS_LOCK_PLUG_RESPONSE_OK);
}

static void switch_user_button_clicked(GtkButton* button, GSLockPlug* plug) {
	remove_response_idle(plug);
...
static int delete_handler(GSLockPlug* plug, GdkEventAny* event, gpointer data) {
	gs_lock_plug_response(plug, GS_LOCK_PLUG_RESPONSE_CANCEL);
...
static void gs_lock_plug_init(GSLockPlug* plug) {
	...
	g_signal_connect(plug, "key_press_event", G_CALLBACK(entry_key_press), plug);

	/* button press handler used to inhibit popup menu */
	g_signal_connect(plug->priv->auth_prompt_entry, "button_press_event", G_CALLBACK(entry_button_press), NULL);
	...
	g_signal_connect(plug->priv->auth_unlock_button, "clicked", G_CALLBACK(unlock_button_clicked), plug);
	...
	if (plug->priv->auth_switch_button != NULL) {
		g_signal_connect(plug->priv->auth_switch_button, "clicked", G_CALLBACK(switch_user_button_clicked), plug);
	}

	if (plug->priv->auth_logout_button != NULL) {
		g_signal_connect(plug->priv->auth_logout_button, "clicked", G_CALLBACK(logout_button_clicked), plug);
	}
...
static void gs_lock_plug_finalize(GObject* object) {
	...
	if (plug->priv->input_sources != NULL) g_ptr_array_unref(plug->priv->input_sources);
