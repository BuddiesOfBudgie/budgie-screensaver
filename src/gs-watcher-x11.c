...
static void add_watchdog_timer(GSWatcher* watcher, glong timeout) {
	watcher->priv->watchdog_timer_id = g_timeout_add_seconds(
		timeout, (GSourceFunc) watchdog_timer, watcher
	);
}
...
static void gs_watcher_class_init(GSWatcherClass* klass) {
	...
	signals[IDLE_CHANGED] = g_signal_new(
		"idle-changed",
	...
	signals[IDLE_NOTICE_CHANGED] = g_signal_new(
		"idle-notice-changed",
		G_TYPE_FROM_CLASS (object_class),
		G_SIGNAL_RUN_LAST,
...
static void set_status(GSWatcher* watcher, guint status) {
	...
	if (is_idle) {
		...
		watcher->priv->idle_id = g_timeout_add_seconds(
...
static void connect_presence_watcher(GSWatcher* watcher) {
	...
	watcher->priv->presence_proxy = dbus_g_proxy_new_for_name(
		bus,
		GSM_SERVICE,
		GSM_PRESENCE_PATH,
	...
	proxy = dbus_g_proxy_new_from_proxy(
		watcher->priv->presence_proxy,
...
static void disable_builtin_screensaver(GSWatcher* watcher, gboolean unblank_screen) {
	...
	*/
	...
	if (desired_server_timeout != current_server_timeout || desired_server_interval != current_server_interval
		...
		gs_debug (
			"disabling server builtin screensaver: (xset s %d %d; xset s %s; xset s %s)",
