...
enum { PROP_0, PROP_STATUS_MESSAGE };

enum { IDLE_CHANGED, IDLE_NOTICE_CHANGED, LAST_SIGNAL };
...
static void add_watchdog_timer(GSWatcher* watcher, glong timeout) {
	watcher->priv->watchdog_timer_id = g_timeout_add_seconds(timeout, (GSourceFunc) watchdog_timer, watcher);
}

static void gs_watcher_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
	GSWatcher* self;
...
static void set_status_text(GSWatcher* watcher, const char* text) {
...
static void gs_watcher_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
	GSWatcher* self;
...
static void gs_watcher_class_init(GSWatcherClass* klass) {
	...
	signals[IDLE_CHANGED] = g_signal_new("idle-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSWatcherClass, idle_changed), NULL, NULL, gs_marshal_BOOLEAN__BOOLEAN, G_TYPE_BOOLEAN, 1,
		G_TYPE_BOOLEAN);
	signals[IDLE_NOTICE_CHANGED] = g_signal_new("idle-notice-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(GSWatcherClass, idle_notice_changed), NULL, NULL, gs_marshal_BOOLEAN__BOOLEAN, G_TYPE_BOOLEAN, 1,
		G_TYPE_BOOLEAN);

	g_type_class_add_private(klass, sizeof(GSWatcherPrivate));
}

static gboolean _gs_watcher_set_session_idle_notice(GSWatcher* watcher, gboolean in_effect) {
	gboolean res;
...
static gboolean _gs_watcher_set_session_idle(GSWatcher* watcher, gboolean is_idle) {
	gboolean res;
...
static void _gs_watcher_reset_state(GSWatcher* watcher) {
	watcher->priv->idle = FALSE;
	watcher->priv->idle_notice = FALSE;
}

static gboolean _gs_watcher_set_active_internal(GSWatcher* watcher, gboolean active) {
...
gboolean gs_watcher_set_active(GSWatcher* watcher, gboolean active) {
	...
	if (watcher->priv->active == active) {
		gs_debug("Idle detection is already %s", active ? "active" : "inactive");
		return FALSE;
	}
...
static gboolean on_idle_timeout(GSWatcher* watcher) {
	gboolean res;
...
static void set_status(GSWatcher* watcher, guint status) {
	gboolean is_idle;
	...
	if (is_idle) {
		...
		watcher->priv->idle_id =
			g_timeout_add_seconds(watcher->priv->delta_notice_timeout, (GSourceFunc) on_idle_timeout, watcher);
	} else {
		/* cancel notice too */
		if (watcher->priv->idle_id > 0) {
			g_source_remove(watcher->priv->idle_id);
...
static void on_presence_status_changed(DBusGProxy* presence_proxy, guint status, GSWatcher* watcher) {
	set_status(watcher, status);
}

static void on_presence_status_text_changed(DBusGProxy* presence_proxy, const char* status_text, GSWatcher* watcher) {
	set_status_text(watcher, status_text);
}

static void connect_presence_watcher(GSWatcher* watcher) {
	DBusGConnection* bus;
	GError* error;
	DBusGProxy* proxy;
	guint status;
	...
	watcher->priv->presence_proxy = dbus_g_proxy_new_for_name(bus, GSM_SERVICE, GSM_PRESENCE_PATH, GSM_PRESENCE_INTERFACE);

	dbus_g_proxy_add_signal(watcher->priv->presence_proxy, "StatusChanged", G_TYPE_UINT, G_TYPE_INVALID);
	dbus_g_proxy_connect_signal(
		watcher->priv->presence_proxy, "StatusChanged", G_CALLBACK(on_presence_status_changed), watcher, NULL);
	dbus_g_proxy_add_signal(watcher->priv->presence_proxy, "StatusTextChanged", G_TYPE_STRING, G_TYPE_INVALID);
	dbus_g_proxy_connect_signal(
		watcher->priv->presence_proxy, "StatusTextChanged", G_CALLBACK(on_presence_status_text_changed), watcher, NULL);

	proxy = dbus_g_proxy_new_from_proxy(watcher->priv->presence_proxy, "org.freedesktop.DBus.Properties", GSM_PRESENCE_PATH);

	status = 0;
	status_text = NULL;

	error = NULL;
	dbus_g_proxy_call(proxy, "Get", &error, G_TYPE_STRING, GSM_PRESENCE_INTERFACE, G_TYPE_STRING, "status", G_TYPE_INVALID,
	...
	error = NULL;
	dbus_g_proxy_call(proxy, "Get", &error, G_TYPE_STRING, GSM_PRESENCE_INTERFACE, G_TYPE_STRING, "status-text", G_TYPE_INVALID,
		G_TYPE_VALUE, &value, G_TYPE_INVALID);
...
*/
static void disable_builtin_screensaver(GSWatcher* watcher, gboolean unblank_screen) {
	int current_server_timeout, current_server_interval;
	int current_prefer_blank, current_allow_exp;
	int desired_server_timeout, desired_server_interval;
	int desired_prefer_blank, desired_allow_exp;

	XGetScreenSaver(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), &current_server_timeout, &current_server_interval,
	...
	*/
	...
	if (desired_server_timeout != current_server_timeout || desired_server_interval != current_server_interval ||
		desired_prefer_blank != current_prefer_blank || desired_allow_exp != current_allow_exp) {
		gs_debug(
			"disabling server builtin screensaver:"
			" (xset s %d %d; xset s %s; xset s %s)",
			desired_server_timeout, desired_server_interval, (desired_prefer_blank ? "blank" : "noblank"),
			(desired_allow_exp ? "expose" : "noexpose"));

		XSetScreenSaver(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), desired_server_timeout, desired_server_interval,
...
GSWatcher* gs_watcher_new(void) {
	...
	watcher = g_object_new(GS_TYPE_WATCHER, NULL);
