...
enum {
	DIALOG_RESPONSE_CANCEL, DIALOG_RESPONSE_OK
};
...
enum {
	ACTIVITY, DEACTIVATED, LAST_SIGNAL
};
...
static void update_geometry(GSWindow* window) {
	...
	gdk_monitor_get_geometry(monitor, &geometry);
	gs_debug (
		"got geometry for monitor %d: x=%d y=%d w=%d h=%d",
		window->priv->monitor,
		geometry.x,
		geometry.y,
		geometry.width,
	...
	gs_debug (
		"using geometry for monitor %d: x=%d y=%d w=%d h=%d",
		window->priv->monitor,
		geometry.x,
		geometry.y,
		geometry.width,
...
static void gs_window_move_resize_window(GSWindow* window, gboolean move, gboolean resize) {
	...
	gs_debug (
		"Move and/or resize window on monitor %d: x=%d y=%d w=%d h=%d",
		window->priv->monitor,
	...
	GdkDisplay* default_display = gdk_display_get_default();
	monitor = gdk_display_get_monitor_at_window(default_display, gtk_widget_get_window(widget));
...
wait_again:
	if (waitpid(pid, &status, 0) < 0) {
		if (errno == EINTR) {
			goto wait_again;
		} else if (errno == ECHILD) { ; /* do nothing, child already reaped */
		} else {
...
static void embed_keyboard(GSWindow* window) {
	...
	if (!window->priv->keyboard_enabled || window->priv->keyboard_command == NULL) {
		return;
	}
...
static void queue_key_event(GSWindow* window, GdkEventKey* event) {
	...
	if (window->priv->key_events == NULL
		&& (event->keyval == GDK_KEY_Return || event->keyval == GDK_KEY_KP_Enter || event->keyval == GDK_KEY_Escape
...
static gboolean gs_window_real_grab_broken(GtkWidget* widget, GdkEventGrabBroken* event) {
	...
	if (event->grab_window != NULL) {
		gs_debug (
			"Grab broken on window %X %s, new grab on window %X",
		...
		gs_debug (
			"Grab broken on window %X %s, new grab is outside application",
			(guint32) GDK_WINDOW_XID(event->window),
			event->keyboard ? "keyboard" : "pointer"
		);
	}
...
static void gs_window_class_init(GSWindowClass* klass) {
	...
	g_object_class_install_property(
		object_class, PROP_OBSCURED, g_param_spec_boolean(
			"obscured", NULL, NULL, FALSE, G_PARAM_READABLE
		));
	g_object_class_install_property(
		object_class, PROP_DIALOG_UP, g_param_spec_boolean(
			"dialog-up", NULL, NULL, FALSE, G_PARAM_READABLE
		));
	g_object_class_install_property(
		object_class, PROP_LOCK_ENABLED, g_param_spec_boolean(
			"lock-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_LOGOUT_ENABLED, g_param_spec_boolean(
			"logout-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_LOGOUT_TIMEOUT, g_param_spec_long(
			"logout-timeout", NULL, NULL, -1, G_MAXLONG, 0, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_LOGOUT_COMMAND, g_param_spec_string(
			"logout-command", NULL, NULL, NULL, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_STATUS_MESSAGE, g_param_spec_string(
			"status-message", NULL, NULL, NULL, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_KEYBOARD_ENABLED, g_param_spec_boolean(
			"keyboard-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE
		));
	g_object_class_install_property(
		object_class, PROP_KEYBOARD_COMMAND, g_param_spec_string(
			"keyboard-command", NULL, NULL, NULL, G_PARAM_READWRITE
		));

	g_object_class_install_property(
		object_class, PROP_MONITOR, g_param_spec_int(
			"monitor",
			"Xinerama monitor",
...
static void update_clock(GSWindow* window) {
	...
	markup = g_strdup_printf(
		"<b><span foreground=\"#ccc\">%s</span></b>",
...
static void gs_window_init(GSWindow* window) {
	...
	gtk_widget_set_events(GTK_WIDGET (window),
		gtk_widget_get_events(GTK_WIDGET (window)) | GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK
