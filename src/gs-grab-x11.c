...
static int gs_grab_get_keyboard(GSGrab* grab, GdkWindow* window, GdkScreen* screen) {
	...
	if (status == GDK_GRAB_SUCCESS) {
		if (grab->priv->keyboard_grab_window != NULL) {
			g_object_remove_weak_pointer(
				G_OBJECT(grab->priv->keyboard_grab_window), (gpointer*) &grab->priv->keyboard_grab_window);
		}
		grab->priv->keyboard_grab_window = window;

		g_object_add_weak_pointer(G_OBJECT(grab->priv->keyboard_grab_window), (gpointer*) &grab->priv->keyboard_grab_window);
...
static int gs_grab_get_mouse(GSGrab* grab, GdkWindow* window, GdkScreen* screen, gboolean hide_cursor) {
	GdkGrabStatus status;
	GdkCursor* cursor;
	...
	status = gdk_pointer_grab(window, TRUE, 0, NULL, (hide_cursor ? cursor : NULL), GDK_CURRENT_TIME);

	if (status == GDK_GRAB_SUCCESS) {
		if (grab->priv->mouse_grab_window != NULL) {
			g_object_remove_weak_pointer(G_OBJECT(grab->priv->mouse_grab_window), (gpointer*) &grab->priv->mouse_grab_window);
		}
		grab->priv->mouse_grab_window = window;

		g_object_add_weak_pointer(G_OBJECT(grab->priv->mouse_grab_window), (gpointer*) &grab->priv->mouse_grab_window);
...
void gs_grab_keyboard_reset(GSGrab* grab) {
	if (grab->priv->keyboard_grab_window != NULL) {
		g_object_remove_weak_pointer(G_OBJECT(grab->priv->keyboard_grab_window), (gpointer*) &grab->priv->keyboard_grab_window);
	}
	grab->priv->keyboard_grab_window = NULL;
	grab->priv->keyboard_grab_screen = NULL;
}

static gboolean gs_grab_release_keyboard(GSGrab* grab) {
...
static gboolean gs_grab_move_mouse(GSGrab* grab, GdkWindow* window, GdkScreen* screen, gboolean hide_cursor) {
	gboolean result;
	...
	if (grab->priv->mouse_grab_window == window) {
		gs_debug("Window %X is already grabbed, skipping", (guint32) GDK_WINDOW_XID(grab->priv->mouse_grab_window));
		return TRUE;
	}
	...
	if (grab->priv->mouse_grab_window) {
		gs_debug("Moving pointer grab from %X to %X", (guint32) GDK_WINDOW_XID(grab->priv->mouse_grab_window),
...
static gboolean gs_grab_move_keyboard(GSGrab* grab, GdkWindow* window, GdkScreen* screen) {
	gboolean result;
	GdkWindow* old_window;
	GdkScreen* old_screen;

	if (grab->priv->keyboard_grab_window == window) {
		gs_debug("Window %X is already grabbed, skipping", (guint32) GDK_WINDOW_XID(grab->priv->keyboard_grab_window));
		return TRUE;
	}

	if (grab->priv->keyboard_grab_window != NULL) {
		gs_debug("Moving keyboard grab from %X to %X", (guint32) GDK_WINDOW_XID(grab->priv->keyboard_grab_window),
			(guint32) GDK_WINDOW_XID(window));
	} else {
		gs_debug("Getting keyboard grab on %X", (guint32) GDK_WINDOW_XID(window));
	}
...
static void request_shell_exit_overview(GSGrab* grab) {
	...
	if (!grab->priv->session_bus) return;

	message = g_dbus_message_new_method_call("org.gnome.Shell", "/org/gnome/Shell", "org.freedesktop.DBus.Properties", "Set");
	g_dbus_message_set_body(message, g_variant_new("(ssv)", "org.gnome.Shell", "OverviewActive", g_variant_new("b", FALSE)));

	g_dbus_connection_send_message(grab->priv->session_bus, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
	g_object_unref(message);
}

gboolean gs_grab_grab_window(GSGrab* grab, GdkWindow* window, GdkScreen* screen, gboolean hide_cursor) {
...
AGAIN:
	...
	if (mstatus != GDK_GRAB_SUCCESS) {
		gs_debug("Couldn't grab pointer!  (%s)", grab_string(mstatus));
	}
...
void gs_grab_move_to_window(GSGrab* grab, GdkWindow* window, GdkScreen* screen, gboolean hide_cursor) {
...
GSGrab* gs_grab_new(void) {
	if (grab_object) {
		...
		g_object_add_weak_pointer(grab_object, (gpointer*) &grab_object);
	}
