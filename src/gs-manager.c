...
static void add_lock_timer(GSManager* manager, glong timeout) {
	manager->priv->lock_timeout_id = g_timeout_add(
		timeout, (GSourceFunc) activate_lock_timeout, manager
	);
}
...
static void gs_manager_class_init(GSManagerClass* klass) {
	...
	signals[AUTH_REQUEST_BEGIN] = g_signal_new(
		"auth-request-begin",
		G_TYPE_FROM_CLASS (object_class),
		G_SIGNAL_RUN_LAST,
	...
	signals[AUTH_REQUEST_END] = g_signal_new(
		"auth-request-end",
		G_TYPE_FROM_CLASS (object_class),
		G_SIGNAL_RUN_LAST,
	...
	g_object_class_install_property(
		object_class, PROP_ACTIVE, g_param_spec_boolean(
			"active", NULL, NULL, FALSE, G_PARAM_READABLE
		));
	g_object_class_install_property(
	...
	g_object_class_install_property(
		object_class, PROP_LOCK_TIMEOUT, g_param_spec_long(
			"lock-timeout", NULL, NULL, -1, G_MAXLONG, 0, G_PARAM_READWRITE
		));
	g_object_class_install_property(
	...
	g_object_class_install_property(
		object_class, PROP_USER_SWITCH_ENABLED, g_param_spec_boolean(
			"user-switch-enabled", NULL, NULL, FALSE, G_PARAM_READWRITE
		));
...
static gboolean background_settings_change_event_cb(
	GSettings* settings,
	gpointer keys,
	gint n_keys,
	GSManager* manager
	...
	gnome_bg_load_from_preferences(
		manager->priv->bg, manager->priv->settings
	);
...
static gboolean manager_maybe_grab_window(GSManager* manager, GSWindow* window) {
	...
	if (gs_window_get_screen(window) == screen
		...
		gs_grab_move_to_window(
			manager->priv->grab, gs_window_get_gdk_window(window), FALSE);
		grabbed = TRUE;
	}
...
static void handle_window_dialog_down(GSManager* manager, GSWindow* window) {
	...
	/* Regrab the mouse */
	gs_grab_move_to_window(manager->priv->grab, gs_window_get_gdk_window(window), FALSE);
...
static void gs_manager_create_window_for_monitor(GSManager* manager, GdkScreen* screen, int monitor_index) {
	GSWindow* window;
	GdkRectangle rect;
	...
	gs_debug ("Creating window for monitor %d [%d,%d] (%dx%d)", monitor_index, rect.x, rect.y, rect.width, rect.height);
...
static void gs_manager_destroy_windows(GSManager* manager) {
	...
	g_signal_handlers_disconnect_by_func (gdk_display_get_default_screen(display), on_screen_monitors_changed, manager);
